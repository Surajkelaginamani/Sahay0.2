...
export default function PatientQueueCard({ appt, isSelected, onSelect }) {
  ...
  return (
    ...
    >
      ...
      {/* ── Captured Vitals preview if available ───────────────────────── */}
      {appt.vitals && Object.values(appt.vitals).some((v) => v !== null && v !== undefined && String(v).trim().length > 0) && (
        <div className="flex flex-wrap items-center gap-1 mb-1.5 text-[9.5px]">
          {appt.teleconsultSource === 'ASHA' && (
            <span className="px-1.5 py-0.5 rounded bg-violet-100 text-violet-800 font-bold border border-violet-200">
              ASHA Vitals
            </span>
          )}
          {appt.vitals.bloodPressure && (
            <span className="px-1.5 py-0.5 rounded bg-rose-50 text-rose-700 font-semibold border border-rose-200/80">
              BP: {appt.vitals.bloodPressure}
            </span>
          )}
          {appt.vitals.pulse && (
            <span className="px-1.5 py-0.5 rounded bg-sky-50 text-sky-700 font-semibold border border-sky-200/80">
              P: {appt.vitals.pulse}
            </span>
          )}
          {appt.vitals.spO2 && (
            <span className="px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-700 font-semibold border border-emerald-200/80">
              SpO2: {appt.vitals.spO2}%
            </span>
          )}
          {appt.vitals.temperature && (
            <span className="px-1.5 py-0.5 rounded bg-amber-50 text-amber-700 font-semibold border border-amber-200/80">
              {appt.vitals.temperature}°F
            </span>
          )}
          {appt.vitals.bloodSugar && (
            <span className="px-1.5 py-0.5 rounded bg-purple-50 text-purple-700 font-semibold border border-purple-200/80">
              BS: {appt.vitals.bloodSugar}
            </span>
          )}
        </div>
      )}
